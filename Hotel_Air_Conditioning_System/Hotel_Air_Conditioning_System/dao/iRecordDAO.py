...
from sqlalchemy import EnumClass
class iRecordDAO(object):
    ...
    def AddRecord(self, room_id, speed, action_type):
        ...
        session.add(iRecord(invoice_id = invoice_id, start_time = DateTime.now(), trg_tmp = EnumClass(trg_tmp), speed = speed, action_type = EnumClass(action_type), fee_rate = fee_rate))
        session.commit()
