...
class StartUPController(object):
    ...
    def PowerOn(self):
        if gDict["server_state"] != "off":
            return json.dumps({"state":"Invalid Request"})
        # 初始化功能模块
        self.CreateServPool()
        self.CreateSchedule()
        #########不需要
        ...
        ## 读取房间列表
        room_list = json.loads(open("rooms.json").read())
        for room in room_list:
            gDict["rooms"].new_room(room["room_id"], room["trg_tmp"])
    ...
    def SetPara(self, mode, temp_highLimit, temp_lowLimit, default_targetTemp, feeRate_H, feeRate_M,feeRate_L):
        if gDict["server_state"] != ("on" or "start"):
            return json.dumps({"state":"Invalid Request"})
